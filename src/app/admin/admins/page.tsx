...
export default function UsersPage() {
    ...
    const filteredUsers = users.filter(u => {
        const isActuallySuperAdmin = currentUser?.role === 'SUPER_ADMIN' && currentUser?.email === SUPER_ADMIN_EMAIL;
        
        // Only show SUPER_ADMIN if it's the designated email
        if (u.role === 'SUPER_ADMIN') {
            if (u.email !== SUPER_ADMIN_EMAIL) return false; // Hide other superadmins (they shouldn't exist)
            if (!isActuallySuperAdmin) return false; // Hide the real superadmin from everyone else
        }
    ...
    return (
        <div style={{ paddingBottom: 80 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 32, flexWrap: "wrap", gap: 16 }}>
                <div>
                    <h2 style={{ fontSize: 26, fontWeight: 800, color: "#fff", margin: 0, letterSpacing: "-0.03em" }}>Admin Management</h2>
                    <p style={{ fontSize: 12, color: "rgba(255,255,255,0.35)", marginTop: 4, fontWeight: 500 }}>Manage admin users, roles, permissions & team assignments</p>
                </div>
                    ...
                    <tbody>
                        ...
                        {filteredUsers.map((u) => (
                            ...
                            >
                                ...
                                <td style={{ padding: "14px 20px" }}>
                                    {currentUser && u.role !== "SUPER_ADMIN" && ( (currentUser.role === "SUPER_ADMIN" && currentUser.email === SUPER_ADMIN_EMAIL) || ROLES.indexOf(currentUser.role) <= ROLES.indexOf(u.role)) && (
                    ...
                    <div style={{ background: "#141414", borderRadius: 24, padding: 32, width: "100%", maxWidth: 520, maxHeight: "90vh", overflowY: "auto", border: "1px solid rgba(255,255,255,0.08)" }} onClick={(e) => e.stopPropagation()}>
                        ...
                        <select style={inputStyle} value={form.role} onChange={(e: any) => setForm({ ...form, role: e.target.value })}>
                            {ROLES.filter(role => {
                                ...
                                // Only the designated email can ever have the SUPER_ADMIN role
                                if (role === 'SUPER_ADMIN') {
                                    return isActuallySuperAdmin && form.email === SUPER_ADMIN_EMAIL;
                                }
                                
                                // Super admin can see/assign all other roles
                                if (isActuallySuperAdmin) return true;
                                
                                // Others can only see roles lower than or equal to theirs
                                return ROLES.indexOf(currentUser?.role || 'USER') <= ROLES.indexOf(role);
                            }).map(role => (
                        ...
                        <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: 20, marginTop: 4 }}>
                            ...
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                                {AVAILABLE_SECTIONS.map((section) => {
                                    const isActuallySuperAdmin = currentUser?.role === "SUPER_ADMIN" && currentUser?.email === SUPER_ADMIN_EMAIL;
                                    if (!isActuallySuperAdmin && !currentUser?.allowedSections?.includes(section.key)) {
                                        return null;
                                    }
                        ...
                        <div style={{ display: "flex", gap: 12, marginTop: 28, justifyContent: "flex-end" }}>
                            ...
                            <button onClick={handleSave} disabled={saving} style={{ padding: "11px 28px", borderRadius: 10, background: "#00875a", color: "#fff", fontWeight: 700, border: "none", cursor: "pointer", fontSize: 13, opacity: saving ? 0.6 : 1 }}>{saving ? "Saving..." : "Save Admin"}</button>
                        </div>
                    </div>
                </div>
            )}
